...
func TestStore_AddAndReadFrom_SinglePosition(t *testing.T) {
	...
	// 添加事件
	s.Add(NewChunkEvent("hello", "s1"))
	s.Add(NewChunkEvent("world", "s1"))
	s.Add(NewDoneEvent("s1"))
	...
	evt = s.ReadFrom(pos)
	if evt == nil || evt.EventType != EventTypeDone {
...
func TestStore_ReadFrom_StartOffset(t *testing.T) {
	s := NewStore("s1", nil)
	s.Add(NewChunkEvent("a", "s1"))
	s.Add(NewChunkEvent("b", "s1"))
	s.Add(NewChunkEvent("c", "s1"))
...
func TestStore_StartClamped(t *testing.T) {
	s := NewStore("s1", nil)
	s.Add(NewChunkEvent("a", "s1")) // seq=0
...
func TestStore_AppendHistory_SetsStartAndOffset(t *testing.T) {
	...
	// 再添加两个事件
	s.Add(NewChunkEvent("d", "s1"))
	s.Add(NewChunkEvent("e", "s1"))

	msg2 := &Message{Role: RoleUser}
...
func (f *fakeHistoryStore) LoadHistory(_ string) ([]Message, error) {
	if f.err != nil {
		return nil, f.err
	}
...
func (f *fakeHistoryStore) AppendMessages(_ string, _ []Message) error { return nil }

func TestStore_LoadHistory_RestoresSeq(t *testing.T) {
	store := &fakeHistoryStore{
		messages: []Message{
			{Role: RoleUser, Start: 0, Offset: 3, Content: Blocks{NewTextBlock("hello")}},
			{Role: RoleAssistant, Start: 3, Offset: 5, Content: Blocks{NewTextBlock("hi")}},
		},
	}
	s := NewStore("s1", store)
...
func TestStore_LoadHistory_Idempotent(t *testing.T) {
	store := &fakeHistoryStore{
		messages: []Message{
			{Role: RoleUser, Start: 0, Offset: 1, Content: Blocks{NewTextBlock("hi")}},
		},
	}
	s := NewStore("s1", store)
...
type recordingHistoryStore struct {
	appended [][]Message // 记录每次 AppendMessages 的调用
}

func (r *recordingHistoryStore) LoadHistory(_ string) ([]Message, error) { return nil, nil }
func (r *recordingHistoryStore) AppendMessages(_ string, msgs []Message) error {
...
func TestStore_SaveHistory_AppendsNewMessages(t *testing.T) {
	...
	// 添加一条消息
	msg := Message{Role: RoleUser, Content: Blocks{NewTextBlock("hello")}}
	s.AppendHistory(&msg)
	...
	if rec.appended[0][0].Content[0].(*TextBlock).Text != "hello" {
