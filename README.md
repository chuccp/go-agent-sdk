...
- **断线续传** — 消息携带事件流区间 `[Start, Start+Offset)`，客户端凭一个 `start` 值即可精确续读
- **历史持久化** — 内存 + DB 双层存储，增量追加，懒加载
- **多提供商** — Provider Registry 支持注册多个 LLM 后端，运行时选择
- **多客户端订阅** — 同一会话可被多个 Client 同时订阅（多标签页）
...
```
go-agent-sdk/
├── agent/          # Agent 层：ChatManager, ChatSession, ChatClient, Tool, Options
├── chat/           # 协议层：Block, Event, Message, Request/Response, Store, Provider
...
```
用户消息 → build(): 入 history, 发 Consumed 事件
         → streamResponse(): 流式事件累积
         → assistant msg 入 history, 回填 Offset
         → [tool_use]: executeTools → tool_result 入 history
         → end_turn: SaveHistory(增量写 DB) → Reset(清空 entries, 推进 base)
```
...
- **零基础设施断线续传** — 消息自带 `[Start, Offset)` 区间（类 Kafka consumer offset），无需外部 broker
- **Store 双区模型** — entries（易失）+ history（持久），`Reset()` 只推进 base，零 GC
- **协议/推送事件分离** — 换 LLM 提供商只改协议适配，前端推送协议不动
- **工具池快照隔离** — Session 创建时复制工具快照，运行期间 AddTool 无 race
- **blockCollector 自动 flush** — "新 block 开始 = 上一个自动关闭"，无显式状态机
