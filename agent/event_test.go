...
// TestSignalEvents_BasicFlow 验证信号事件的基本读取流程。
func TestSignalEvents_BasicFlow(t *testing.T) {
	tr := newTestTransfer()

	// 添加信号事件（不占用 start 序号）
	tr.signalEvents.Append(&Event{Start: 100, Offset: 1, Blocks: chat.Blocks{chat.NewFullTextBlock("signal1")}})
	tr.signalEvents.Append(&Event{Start: 101, Offset: 1, Blocks: chat.Blocks{chat.NewFullTextBlock("signal2")}})

	// 创建客户端，signalStart=0
	cl := &Client{start: 0, signalStart: 0, readEvents: tr}

	// 读取信号事件
	events, err := tr.readSignalEvents(cl)
	if err != nil {
		t.Fatalf("readSignalEvents error: %v", err)
	}
	if len(events) != 2 {
		t.Fatalf("readSignalEvents = %d events, want 2", len(events))
	}
	if events[0].Start != 100 || events[1].Start != 101 {
		t.Errorf("got Start=[%d,%d], want [100,101]", events[0].Start, events[1].Start)
	}

	// 验证 cl.signalStart 已更新
	if cl.signalStart != 102 {
		t.Fatalf("cl.signalStart = %d, want 102", cl.signalStart)
	}
}

// TestSignalEvents_NoNewEvents 验证没有新信号事件时返回空。
func TestSignalEvents_NoNewEvents(t *testing.T) {
	tr := newTestTransfer()

	// 添加信号事件
	tr.signalEvents.Append(&Event{Start: 100, Offset: 1, Blocks: chat.Blocks{chat.NewFullTextBlock("signal1")}})

	// 创建客户端，signalStart 已经超过信号事件
	cl := &Client{start: 0, signalStart: 101, readEvents: tr}
	...
	if len(events) != 0 {
		t.Fatalf("readSignalEvents = %d events, want 0", len(events))
	}
}

// TestSignalEvents_MultipleClients 验证多个客户端读取信号事件。
func TestSignalEvents_MultipleClients(t *testing.T) {
	tr := newTestTransfer()
	...
	// 创建两个客户端
	cl1 := &Client{start: 0, signalStart: 0, readEvents: tr}
	cl2 := &Client{start: 0, signalStart: 0, readEvents: tr}
	tr.chatClients.Append(cl1)
	tr.chatClients.Append(cl2)

	// 客户端1读取信号事件
	events1, err := tr.readSignalEvents(cl1)
	if err != nil {
		t.Fatalf("readSignalEvents cl1 error: %v", err)
	}
	if len(events1) != 2 {
		t.Fatalf("readSignalEvents cl1 = %d events, want 2", len(events1))
	}

	// 信号事件应该还在（cl2 还没读）
	if tr.signalEvents.Len() != 2 {
		t.Fatalf("signalEvents.Len() = %d, want 2", tr.signalEvents.Len())
	}

	// 客户端2读取信号事件
	events2, err := tr.readSignalEvents(cl2)
	if err != nil {
		t.Fatalf("readSignalEvents cl2 error: %v", err)
	}
	if len(events2) != 2 {
		t.Fatalf("readSignalEvents cl2 = %d events, want 2", len(events2))
	}

	// 所有客户端都已读取，信号事件应该被清理
	if tr.signalEvents.Len() != 0 {
		t.Fatalf("signalEvents.Len() after cleanup = %d, want 0", tr.signalEvents.Len())
	}
}

// TestSignalEvents_PartialCleanup 验证部分客户端读取时不会清理信号事件。
func TestSignalEvents_PartialCleanup(t *testing.T) {
	tr := newTestTransfer()
	...
	// 只有客户端1读取
	_, err := tr.readSignalEvents(cl1)
	if err != nil {
		t.Fatalf("readSignalEvents cl1 error: %v", err)
	}

	// 信号事件应该还在（cl2 还没读）
	if tr.signalEvents.Len() != 1 {
		t.Fatalf("signalEvents.Len() = %d, want 1", tr.signalEvents.Len())
	}
}

// TestSignalEvents_ClosedClientIgnored 验证已关闭的客户端不会阻止信号事件清理。
func TestSignalEvents_ClosedClientIgnored(t *testing.T) {
	tr := newTestTransfer()
	...
	// 创建两个客户端，其中一个已关闭
	cl1 := &Client{start: 0, signalStart: 0, readEvents: tr}
	cl2 := &Client{start: 0, signalStart: 0, readEvents: tr, isClosed: true}
	tr.chatClients.Append(cl1)
	tr.chatClients.Append(cl2)
	...
	// 信号事件应该被清理（cl2 已关闭，不计入）
	if tr.signalEvents.Len() != 0 {
		t.Fatalf("signalEvents.Len() after cleanup = %d, want 0", tr.signalEvents.Len())
	}
}

// TestSignalEvents_DifferentSignalStart 验证不同 signalStart 的客户端。
func TestSignalEvents_DifferentSignalStart(t *testing.T) {
	tr := newTestTransfer()

	// 添加多个信号事件
	tr.signalEvents.Append(&Event{Start: 100, Offset: 1, Blocks: chat.Blocks{chat.NewFullTextBlock("signal1")}})
	tr.signalEvents.Append(&Event{Start: 101, Offset: 1, Blocks: chat.Blocks{chat.NewFullTextBlock("signal2")}})
	tr.signalEvents.Append(&Event{Start: 102, Offset: 1, Blocks: chat.Blocks{chat.NewFullTextBlock("signal3")}})

	// 创建两个客户端，不同的 signalStart
	cl1 := &Client{start: 0, signalStart: 0, readEvents: tr}
	cl2 := &Client{start: 0, signalStart: 101, readEvents: tr} // 已经读过前两个
	tr.chatClients.Append(cl1)
	tr.chatClients.Append(cl2)
	...
	if len(events1) != 3 {
		t.Fatalf("readSignalEvents cl1 = %d events, want 3", len(events1))
	}
