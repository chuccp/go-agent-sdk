...
func TestToolArgsDisplay_CommandField(t *testing.T) {
	got := toolArgsDisplay(map[string]any{"command": "ls -la", "cwd": "/tmp"})
	if got != "ls -la" {
...
func TestToolArgsDisplay_EmptyCommand(t *testing.T) {
	got := toolArgsDisplay(map[string]any{"command": "", "other": "val"})
	if got == "" {
...
func TestToolArgsDisplay_NoCommand(t *testing.T) {
	got := toolArgsDisplay(map[string]any{"key": "value"})
	expected := `{"key":"value"}`
	if got != expected {
...
func TestToolArgsDisplay_EmptyArgs(t *testing.T) {
	got := toolArgsDisplay(map[string]any{})
	if got != "" {
...
func TestTurn_Context_Nil(t *testing.T) {
	turn := NewTurn(map[string]any{"x": "y"})
	if turn.Context() != nil {
...
func TestNewTurn(t *testing.T) {
	turn := NewTurn(map[string]any{"key": "value"})
	if turn.Args()["key"] != "value" {
...
func (f *fakeTool) Execute(_ *Turn, _ *BlockStream) error { return nil }
